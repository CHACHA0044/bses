...
export default function ConnectionHistoryPage() {
  // SWR-backed — the idle PrefetchProvider warms /connections so this page
  // renders instantly when navigated to from the dashboard.
  const { data, loading } = useApiResource<{ connections?: any[] }>('/connections');
  const connections = data?.connections || [];

  if (loading) {
    return <div className="p-8 text-slate-500 animate-pulse">Loading service requests...</div>;
  }
